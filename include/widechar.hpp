...
DUCKDB_API std::vector<SQLCHAR> utf16_to_utf8_lenient(const SQLWCHAR *in_buf, size_t in_buf_len,
                                           const SQLWCHAR **first_invalid_char = nullptr);

DUCKDB_API std::vector<SQLWCHAR> utf8_to_utf16_lenient(const SQLCHAR *in_buf, size_t in_buf_len,
                                                       const SQLCHAR **first_invalid_char = nullptr);

DUCKDB_API size_t utf16_length(const SQLWCHAR *buf);

DUCKDB_API size_t utf16_length(const std::string &utf8_str);

DUCKDB_API std::vector<SQLCHAR> utf8_alloc_out_vec(SQLSMALLINT utf16_len);

DUCKDB_API size_t utf16_write_str(SQLRETURN ret, const std::vector<SQLCHAR> &utf8_vec, SQLWCHAR *utf16_str_out,
