...
TEST_CASE("Test SQLColAttribute for a query that returns a uuid", "[odbc]") {
	SQLHANDLE env;
	SQLHANDLE dbc;
	...
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT gen_random_uuid() AS a, uuid() AS b"), SQL_NTS);
	...
	expected_chars[SQL_DESC_SEARCHABLE] = ExpectedResult(SQL_ALL_EXCEPT_LIKE);
