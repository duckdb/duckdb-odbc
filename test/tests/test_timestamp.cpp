...
TEST_CASE("Test SQLBindParameter with TIMESTAMP type", "[odbc]") {
	...
	// Insert value
	EXECUTE_AND_CHECK("SQLPrepare", SQLPrepare, hstmt, ConvertToSQLCHAR("INSERT INTO timestamp_bind_test VALUES (?)"),
	                  SQL_NTS);
	SQL_TIMESTAMP_STRUCT param = {2000, 1, 1, 12, 10, 30, 0};
	SQLLEN param_len = sizeof(param);
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 1, SQL_PARAM_INPUT, SQL_C_TYPE_TIMESTAMP,
	                  SQL_TYPE_TIMESTAMP, 0, 0, &param, param_len, &param_len);
	EXECUTE_AND_CHECK("SQLExecute", SQLExecute, hstmt);

	// Fetch and check
	EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT * FROM timestamp_bind_test"),
	                  SQL_NTS);
	EXECUTE_AND_CHECK("SQLFetch", SQLFetch, hstmt);
	SQL_TIMESTAMP_STRUCT fetched;
	EXECUTE_AND_CHECK("SQLGetData", SQLGetData, hstmt, 1, SQL_C_TYPE_TIMESTAMP, &fetched, sizeof(fetched), nullptr);
...
TEST_CASE("Test SQLBindParameter with DATE type", "[odbc]") {
	...
	// Insert value
	EXECUTE_AND_CHECK("SQLPrepare", SQLPrepare, hstmt, ConvertToSQLCHAR("INSERT INTO date_bind_test VALUES (?)"),
	                  SQL_NTS);
	SQL_DATE_STRUCT param = {2000, 1, 2};
	SQLLEN param_len = sizeof(param);
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 1, SQL_PARAM_INPUT, SQL_C_TYPE_DATE, SQL_TYPE_DATE,
	                  0, 0, &param, param_len, &param_len);
	EXECUTE_AND_CHECK("SQLExecute", SQLExecute, hstmt);

	{ // Fetch and check as DATE
		EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT * FROM date_bind_test"),
		                  SQL_NTS);
		EXECUTE_AND_CHECK("SQLFetch", SQLFetch, hstmt);
		SQL_DATE_STRUCT fetched;
		EXECUTE_AND_CHECK("SQLGetData", SQLGetData, hstmt, 1, SQL_C_TYPE_DATE, &fetched, sizeof(fetched), nullptr);
		REQUIRE(fetched.year == param.year);
		REQUIRE(fetched.month == param.month);
		REQUIRE(fetched.day == param.day);
	}
...
TEST_CASE("Test SQLBindParameter with TIME type", "[odbc]") {
	...
	// Insert value
	EXECUTE_AND_CHECK("SQLPrepare", SQLPrepare, hstmt, ConvertToSQLCHAR("INSERT INTO time_bind_test VALUES (?)"),
	                  SQL_NTS);
	SQL_TIME_STRUCT param = {12, 34, 56};
	SQLLEN param_len = sizeof(param);
	EXECUTE_AND_CHECK("SQLBindParameter", SQLBindParameter, hstmt, 1, SQL_PARAM_INPUT, SQL_C_TYPE_TIME, SQL_TYPE_TIME,
	                  0, 0, &param, param_len, &param_len);
	EXECUTE_AND_CHECK("SQLExecute", SQLExecute, hstmt);

	{ // Fetch and check as TIME
		EXECUTE_AND_CHECK("SQLExecDirect", SQLExecDirect, hstmt, ConvertToSQLCHAR("SELECT * FROM time_bind_test"),
		                  SQL_NTS);
		EXECUTE_AND_CHECK("SQLFetch", SQLFetch, hstmt);
		SQL_TIME_STRUCT fetched;
		EXECUTE_AND_CHECK("SQLGetData", SQLGetData, hstmt, 1, SQL_C_TYPE_TIME, &fetched, sizeof(fetched), nullptr);
		REQUIRE(fetched.hour == param.hour);
		REQUIRE(fetched.minute == param.minute);
		REQUIRE(fetched.second == param.second);
	}
