...
TEST_CASE("Test utf8_alloc_out_vec function", "[odbc_widechar]") {
	std::vector<uint8_t> empty_vec = utf8_alloc_out_vec(0);
	REQUIRE(empty_vec.size() == 0);
	std::vector<uint8_t> small_vec = utf8_alloc_out_vec(100);
	REQUIRE(small_vec.size() == 300);
	std::vector<uint8_t> large_vec = utf8_alloc_out_vec(30000);
	REQUIRE(large_vec.size() == (std::numeric_limits<int16_t>::max)());
}
...
TEST_CASE("Test utf16_write_str function", "[odbc_widechar]") {
	...
	std::vector<uint8_t> vec = utf8_alloc_out_vec(100);
	...
	SQLSMALLINT len = static_cast<SQLSMALLINT>(hello_bg_utf8.size());
	...
	// len is in-out parameter here, it is rewritten to hold the number of wide chars written
	utf16_write_str(ret, vec, out.data(), &len);
