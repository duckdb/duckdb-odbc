...
#define INTERNAL_CATCH_MSG_LINENR(macroName, messageType, resultDisposition, fname, linenr, ...)                       \
	do {                                                                                                               \
		Catch::AssertionHandler catchAssertionHandler(macroName##_catch_sr,                                            \
		                                              ::Catch::SourceLineInfo(fname.c_str(), linenr),                  \
		                                              Catch::StringRef(), resultDisposition);                          \
		catchAssertionHandler.handleMessage(                                                                           \
		    messageType, (Catch::MessageStream() << __VA_ARGS__ + ::Catch::StreamEndStop()).m_stream.str());           \
		INTERNAL_CATCH_REACT(catchAssertionHandler)                                                                    \
...
///////////////////////////////////////////////////////////////////////////////
#define INTERNAL_CATCH_SKIP_TEST(reason) Catch::getResultCapture().skipTestDuringRun(reason)
...
clara::Parser makeCommandLineParser(ConfigData &config) {
	...
	auto cli =
	    ...
	    Opt(config.startOffset, "startOffset")["--start-offset"]("start offset of tests (absolute test index)") |
	    Opt(config.endOffset, "endOffset")["--end-offset"]("end offset of tests (absolute test index)") |
	    Opt(config.startOffsetPercentage,
	        "startOffsetPercentage")["--start-offset-percentage"]("start offset of tests (percentage of total tests)") |
	    Opt(config.endOffsetPercentage,
	        "endOffsetPercentage")["--end-offset-percentage"]("end offset of tests (percentage of total tests)") |
	    Opt(config.outputSQL, "true|false")["--output-sql"](
...
int Config::startOffset() const {
	return m_data.startOffset;
}
int Config::endOffset() const {
	return m_data.endOffset;
}
int Config::startOffsetPercentage() const {
	return m_data.startOffsetPercentage;
}
int Config::endOffsetPercentage() const {
	return m_data.endOffsetPercentage;
}
bool Config::outputSQL() const {
	return m_data.outputSQL;
}
...
void renderTestProgress(int current_test, int total_tests, std::string next_test) {
	...
	int render_width = 80;
	std::string result = "[" + std::to_string(current_test) + "/" + std::to_string(total_tests) + "] (" +
	                     std::to_string(int(progress * 100)) + "%): " + next_test;
...
void ConsoleReporter::printTotalsDivider(Totals const &totals) {
	if (totals.testCases.total() > 0) {
		...
		std::size_t failedButOkRatio =
		    makeRatio(totals.testCases.failedButOk + totals.skippedTests, totals.testCases.total());
...
#define FAIL_LINE(fname, linenr, ...)                                                                                  \
	INTERNAL_CATCH_MSG_LINENR("FAIL", Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal, fname,      \
	                          linenr, __VA_ARGS__)
#define FAIL_CHECK(...)                                                                                                \
