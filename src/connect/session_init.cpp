...
static const std::size_t SQL_FILE_MAX_SIZE_BYTES = 1 << 20; // 1MB
static const std::string CONN_INIT_MARKER = "/\\*\\s*DUCKDB_CONNECTION_INIT_BELOW_MARKER\\s*\\*/";

static std::vector<std::string> split_by_marker(const std::string &str) {
	std::regex regex(CONN_INIT_MARKER);
...
std::string SessionInit::SQL_FILE_OPTION = "session_init_sql_file";

std::string SessionInit::SQL_FILE_SHA256_OPTION = "session_init_sql_file_sha256";
...
SessionInitSQLFile SessionInit::ReadSQLFile(const std::string &session_init_sql_file,
	...
	if (file_size > SQL_FILE_MAX_SIZE_BYTES) {
		throw FileIOException("Specified session init SQL file size: " + std::to_string(file_size) +
		                      " exceeds max allowed size: " + std::to_string(SQL_FILE_MAX_SIZE_BYTES));
	}
	...
	std::vector<std::string> parts = split_by_marker(orig_file_text);
	if (parts.size() > 2) {
		throw FileIOException("Connection init marker: '" + CONN_INIT_MARKER + "' can only be specified once");
	}
